...
import PulpoClass  from "../modules/pulpo/class/pulpo.class";
...
router.get('/utc/', async function (req, res, next) {
    ...
    try {
        let lectura = await PulpoClass.timeUTC();
...
router.get('/programacion/:refPulpo', async function (req, res, next) {
    ...
    try {
        let programacion = await PulpoClass.showProgramacion(req.body.refPulpo);
...
router.post('/estado/', async function (req, res, next) {
    try {
        let estado : IEstado = await PulpoClass.saveEstado({
...
router.post('/lectura/:refContador', upload.single('photo'), async function (req, res, next) {
    ...
    try {
        let lectura : ILectura = await PulpoClass.saveLectura({
...
router.get('/db/', async function (req, res, next) {
    ...
    try {
        let ok = await PulpoClass.restartDB();
